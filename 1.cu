...
__device__ half scale_op(half value, float scale) {
  return __float2half_rn(__half2float(value) * scale);
}
...
__device__ float exp_op(half value) {
  return expf(__half2float(value));
}
...
__global__ void flashAttentionKernel(const T* q, const T* k, const T* v, T* o,
  ...
  for (int t = threadIdx.x; t < target_seq_len; t += blockDim.x) {
    ...
    for (int s = 0; s < src_seq_len; s++) {
      ...
      T dot = 0;
      for (int d = 0; d < head_dim; d++) {
        dot = add_op(dot, multiply_op(q_vec[d], k_vec[d]));
      }
      
      // 缩放
      dot = scale_op(dot, scale);
      
      // 跟踪最大值用于数值稳定性
      if (greater_op(dot, max_val)) {
        max_val = dot;
      }
    }
    ...
    for (int s = 0; s < src_seq_len; s++) {
      ...
      // 缩放和应用max_val
      dot = scale_op(dot, scale);
      T diff = subtract_op(dot, max_val);
      float exp_val = exp_op(diff);
      sum_exp += exp_val;
    }
    ...
    for (int s = 0; s < src_seq_len; s++) {
      ...
      T diff = subtract_op(dot, max_val);
      float exp_val = exp_op(diff);
      float weight_float = exp_val / sum_exp;
      
      // 加权累加value
      for (int d = 0; d < head_dim; d++) {
        T scaled_v = scale_op(v_vec[d], weight_float);
        attn_output[d] = add_op(attn_output[d], scaled_v);
      }
    }
